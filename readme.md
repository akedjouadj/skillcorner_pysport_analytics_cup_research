...
├── utils.py # Geometric utilities, offside detection, and helper functions
├── data_loader.py # Data loading and preprocessing pipeline
├── xt_grid.py # Expected Threat (xT) spatial grid
├── offense_rating.py # Offensive rating computation (OCR)
├── defense_rating.py # Defensive rating computation (SCR)
└── analyze_offbr_outputs.py # Analysis utilities for OffBR outputs
...
Tracking data is loaded directly from SkillCorner’s public GitHub repositories using `kloppy`, while match metadata files must be downloaded manually and stored in the following directory structure:
...
**Definition**:
```
- POV = √( P(reception) × xT(receiver_position) )
